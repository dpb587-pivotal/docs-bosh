...
- {: #environments } `bosh environments` (Alias: `envs`)
...
- {: #create-env } `bosh create-env manifest.yml [--state path] [-v ...] [-o ...] [--vars-store path]`
...
- {: #alias-env } `bosh alias-env name -e location [--ca-cert=path]`
...
- {: #environment } `bosh -e location environment` (Alias: `env`)
...
- {: #delete-env } `bosh delete-env manifest.yml [--state path] [-v ...] [-o ...] [--vars-store path]`
...
- {: #log-in } `bosh log-in` (Alias: `l`, `login`)
...
- {: #log-out } `bosh log-out` (Alias: `logout`)
...
- {: #stemcells } `bosh -e my-env stemcells` (Alias: `ss`)
...
- {: #upload-stemcell } `bosh -e my-env upload-stemcell location [--sha1=digest] [--fix]` (Alias: `us`)
...
- {: #delete-stemcell } `bosh -e my-env delete-stemcell name/version`
...
- {: #repack-stemcell } `bosh repack-stemcell src.tgz dst.tgz [--name=name] [--version=ver] [--cloud-properties=json-string]`
...
- {: #init-release } `bosh init-release [--git] [--dir=dir]`
...
- {: #create-release } `bosh create-release [--force] [--version=ver] [--timestamp-version] [--final] [--tarball=path] [--dir=dir]` (Alias: `cr`)
...
- {: #finalize-release } `bosh finalize-release release.tgz [--force] [--version=ver] [--dir=dir]`
...
- {: #reset-release } `bosh reset-release [--dir=dir]`
...
- {: #add-blob } `bosh add-blob src-path dst-path`
...
- {: #remove-blob } `bosh remove-blob blob-path`
...
- {: #releases } `bosh -e my-env releases` (Alias: `rs`)
...
- {: #upload-release } `bosh -e my-env upload-release [location] [--version=ver] [--sha1=digest] [--fix]` (Alias: `ur`)
...
- {: #delete-release } `bosh -e my-env delete-release name/version`
...
- {: #export-release } `bosh -e my-env -d my-dep export-release name/version os/version [--dir=dir]`
...
- {: #inspect-release } `bosh -e my-env inspect-release name/version`
...
- {: #configs } `bosh -e my-env configs [--type=my-type] [--name=my-name]`
...
- {: #config } `bosh -e my-env config [id] [--type=my-type] [--name=my-name]`
...
- {: #update-config } `bosh -e my-env update-config config.yml --type=my-type [--name=my-name]`
...
- {: #delete-config } `bosh -e my-env delete-config --type=my-type [--name=my-name]`
...
- {: #cloud-config } `bosh -e my-env cloud-config` (Alias: `cc`)

    Show current cloud config on the Director.

- {: #update-cloud-config } `bosh -e my-env update-cloud-config config.yml [-v ...] [-o ...]` (Alias: `ucc`)
...
- {: #runtime-config } `bosh -e my-env runtime-config` (Alias: `rc`)

    Show current runtime config on the Director.

- {: #update-runtime-config } `bosh -e my-env update-runtime-config config.yml [-v ...] [-o ...]` (Alias: `urc`)
...
- {: #cpi-config } `bosh -e my-env cpi-config`

    Show current CPI config on the Director.

- {: #update-cpi-config } `bosh -e my-env update-cpi-config config.yml [-v ...] [-o ...]`
...
- {: #deployments } `bosh -e my-env deployments` (Alias: `ds`)
...
- {: #deployment } `bosh -e my-env -d my-dep deployment` (Alias: `dep`)
...
- {: #deploy } `bosh -e my-env -d my-dep deploy manifest.yml [-v ...] [-o ...]`
...
- {: #delete-deployment } `bosh -e my-env -d my-dep delete-deployment [--force]` (Alias: `deld`)
...
- {: #instances } `bosh -e my-env [-d my-dep] instances [--ps] [--details] [--vitals] [--failing]` (Alias: `is`)
...
- {: #manifest } `bosh -e my-env -d my-dep manifest` (Alias: `man`)
...
- {: #recreate } `bosh -e my-env -d my-dep recreate [group[/instance-id]] [--skip-drain] [--fix] [--canaries=] [--max-in-flight=] [--dry-run]`
...
- {: #restart } `bosh -e my-env -d my-dep restart [group[/instance-id]] [--skip-drain] [--canaries=] [--max-in-flight=]`
...
- {: #start } `bosh -e my-env -d my-dep start [group[/instance-id]] [--canaries=] [--max-in-flight=]`
...
- {: #stop } `bosh -e my-env -d my-dep stop [group[/instance-id]] [--skip-drain] [--canaries=] [--max-in-flight=]`
...
- {: #ignore } `bosh -e my-env -d my-dep ignore group/instance-id`

    Ignores instance from being affected by other commands such as `bosh deploy`.

- {: #unignore } `bosh -e my-env -d my-dep unignore group/instance-id`

    Unignores instance from being affected by other commands such as `bosh deploy`.

- {: #logs } `bosh -e my-env -d my-dep logs [group[/instance-id]] [--follow] ...`
...
- {: #events } `bosh -e my-env [-d my-dep] events [--* ...]`
...
- {: #event } `bosh -e my-env event id`

    Shows single event details.

- {: #variables } `bosh -e my-env -d my-dep variables` (Alias: `vars`)
...
- {: #vms } `bosh -e my-env [-d my-dep] vms [--vitals]`
...
- {: #delete-vm } `bosh -e my-env -d my-dep delete-vm cid`
...
- {: #disks } `bosh -e my-env -d my-dep disks [--orphaned]`

    Lists disks. Currently only supports `--orphaned` flag.

- {: #attach-disk } `bosh -e my-env -d my-dep attach-disk group/instance-id disk-cid`
...
- {: #delete-disk } `bosh -e my-env -d my-dep delete-disk cid`
...
- {: #ssh } `bosh -e my-env -d my-dep ssh [destination] [-r] [-c=cmd] [--opts=opts] [--gw-* ...]`
...
- {: #scp } `bosh -e my-env -d my-dep scp src/dst:[file] src/dst:[file] [-r] [--gw-* ...]`
...
- {: #errands } `bosh -e my-env -d my-dep errands` (Alias: `es`)
...
- {: #run-errand } `bosh -e my-env -d my-dep run-errand name [--keep-alive] [--when-changed] [--download-logs] [--logs-dir=dir] [--instance=instance-group/instance-id]`
...
- {: #tasks } `bosh -e my-env tasks [--recent[=num]] [--all]` (Alias: `ts`)
...
- {: #task } `bosh -e my-env task id [--debug] [--result] [--event] [--cpi]` (Alias: `t`)
...
- {: #cancel-task } `bosh -e my-env cancel-task id` (Alias: `ct`)
...
- {: #snapshots } `bosh -e my-env -d my-dep snapshots`

    Lists disk snapshots for given deployment.

- {: #take-snapshot } `bosh -e my-env -d my-dep take-snapshot [group/instance-id]`

    Takes snapshot for an instance or an entire deployment.

- {: #delete-snapshot } `bosh -e my-env -d my-dep delete-snapshot cid`
...
- {: #delete-snapshots } `bosh -e my-env -d my-dep delete-snapshots`
...
- {: #update-resurrection } `bosh -e my-env update-resurrection on/off`
...
- {: #clean-up } `bosh -e my-env clean-up [--all]`
...
- {: #interpolate } `bosh interpolate manifest.yml [-v ...] [-o ...] [--vars-store path] [--path op-path]` (Alias: `int`)
