...
The `name` represents the name of the network within the director. It may be different from network names used by the underlying IaaS, and this is how deployments will refer to the network.
...
The `type` must be configured as `manual` for a manual network.
...
The `subnets` list must include all subnets which are available in this network.
...
The `range` must specify the IP range used in this subnet. This should be specified as a CIDR (e.g. `10.0.0.0/24`)
...
The `gateway` must specify the gateway IP for this subnet. This is typically the first IP of the CIDR (e.g. `10.0.0.1`)
...
The `dns` list may include specific DNS IP addresses to use for this subnet.
...
The `reserved` list may include IPs and/or IP ranges which should not be allocated by BOSH. This will typically be used to exclude IaaS-reserved IPs or VM IPs which are managed outside of BOSH.
...
The `static` list must include IPs and/or IP ranges which will be used by deployments that need to explicitly assign IPs to instance groups. Only IPs specified here may be used for static IP reservations.


#### `az`

The `az` should be be used to associate this subnet with a particular [availability zone](azs.md).

 * Required if using multiple availability zones
 * Type: `string`
 * Since: bosh/v241
...
The `cloud_properties` should include any IaaS-specific cloud properties needed for associating resources with this network.

 * Required for most IaaSes
 * Type: `hash`
